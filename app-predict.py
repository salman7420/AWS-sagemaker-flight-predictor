...
with st.sidebar:
    ...
    icons=[ 'house', 'search','airplane','house','globe','clock','calendar'], menu_icon="cast", default_index=1)

#form for prediction:
airline = st.selectbox("Select An Airline:",
                       options = X_train.airline.unique())

date_of_journey = st.date_input("Select Journey Date:")
...
duration = st.number_input("Flight Duration:", 
                        step=1)
...
additional_info = st.selectbox("Enter Additional Information:",
...
if st.button("Predict"):
	joblib_preprocessor = joblib.load("preprocessor.joblib")
	X_new_pre = joblib_preprocessor.transform(X_new)
